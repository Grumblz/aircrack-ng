...
char *user = "aircrack";
char *password = "SDGW#tp9)EJGW#T_";
char *database = "aircrack";
#else
FILE * fp_sql = NULL;
#endif
...
void gmv_log_accesspoint(struct AP_info * ap_cur )
{
  ...
  char mysql_str[256];
  // Save to database table "accesspoint"
...
void gmv_log_client(struct ST_info * st_cur)
{
...
void gmv_log_connection(struct ST_info * st_cur, struct AP_info * ap_cur)
{
  ...
  // o add connection ID to save the close time and date as well
#else  
  fprintf(fp_sql,"%s\n", mysql_str);
  fflush(fp_sql);
#endif
			   ...
			   int cardnum)
{
  ...
  if (st_cur == NULL)
      ...
      for (i = 0; i < NB_PRB; i++)
	{
      ...
      gmv_log_client(st_cur);
      
      lopt.st_end = st_cur;
    } // New station found

  // GMV: todo: log if the connection was changed
